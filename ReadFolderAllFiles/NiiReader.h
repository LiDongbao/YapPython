...
protected:
	...
	template<typename T>
	T* ReverseEndian(T* data, int64_t data_size)
	{
		int type_size = sizeof(T);
		if (type_size == 1) 
			return data;
		char* char_data = new char[type_size];
		for (int shift = 0; shift < data_size; ++shift)
		{
			for (int i = 0; i < type_size; ++i)
			{
				char_data[i] = ((char*)(data + shift))[i];
			}
			for (int i = 0; i < type_size; ++i)
			{
				((char*)(data + shift))[i] = char_data[type_size - i - 1];
			}
		}
		return data;
	};
	
	template<> std::complex<float>* ReverseEndian(std::complex<float>* data, int64_t data_size)
	{
		int type_size = sizeof(std::complex<float>);
		int half_type_size = int(type_size / 2);

		char* char_data = new  char[half_type_size];
		for (int shift = 0; shift < data_size; ++shift)
		{
			// real part
			for (int i = 0; i < half_type_size; ++i)
			{
				char_data[i] = ((char*)(data + shift))[i];
			}
			for (int i = 0; i < half_type_size; ++i)
			{
				((char*)(data + shift))[i] = char_data[half_type_size - i - 1];
			}

			// image part
			for (int i = 0; i < half_type_size; ++i)
			{
				char_data[i] = ((char*)(data + shift))[i + half_type_size];
			}
			for (int i = 0; i < half_type_size; ++i)
			{
				((char*)(data + shift))[i + half_type_size] = char_data[half_type_size - i - 1];
			}
		}
		return data;
	};
