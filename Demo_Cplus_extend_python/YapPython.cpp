...
public:
	...
	virtual void* Process(const wchar_t* module, const wchar_t* method, int data_type, int out_data_type,
		...
		OUT size_t output_size[], bool is_need_ref_data) override;
	...
	virtual void SetRefData(void * ref_data, int32_t data_type, size_t dimension_count, size_t size[]) override 
	{
		...
		try 
		{
			_ref_data_list = CArray2Pylist(ref_data, data_type, dimension_count, size);
		}
...
protected:
	...
	*/
	...
	void pylist2Array(const bpy::object& iterable, T *& out_data, size_t size)
	{
		...
		memcpy(out_data, source_data, (size_t)(size * sizeof(T)));
		out_data += size;
	}
...
void* YapPythonImpl::Process(const wchar_t* module, const wchar_t* method, int data_type, int out_data_type,
	size_t input_dimensions, void * data, OUT size_t& output_dimensions, size_t input_size[],
	OUT size_t output_size[], bool is_need_ref_data)
{
